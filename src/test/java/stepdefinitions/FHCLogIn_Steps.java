...
//    }
    ...
    //HOMEWORK
    @Given("user enter invalid manager username {string}")
    public void user_enter_invalid_manager_username(String string) {
        fhcLoginPage.username.sendKeys(string);
    }
    @Given("user enter invalid manager password {string}")
    public void user_enter_invalid_manager_password(String string) {
        fhcLoginPage.password.sendKeys(string);
    }
    @Then("verify the manager login is not successful")
    public void verify_the_manager_login_is_not_successful() {
        String title=Driver.getDriver().getTitle();
        Assert.assertFalse(title.contains("Admin"));
        Driver.closeDriver();
    }
